...
const useFrameSender = (videoRef) => {
    ...
    const takeFrameAndSend = () => {
        ...
        canvas.toBlob(async (blob) => {
            if (blob) {  // Asegúrese de que blob no sea nulo
                const formData = new FormData();
                formData.append('file', blob, 'frame.webp');

                try {
                    await axios.post('https://5300--main--objectdetection-ar--rodyherrera--c3cp3puaetl6s.pit-1.try.coder.app/api/v1/compare/', formData);
                } catch (error) {
